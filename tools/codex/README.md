# Codex CLI 文档

> OpenAI 的轻量级编码代理，在终端中本地运行

- GitHub: https://github.com/openai/codex
- 文档: https://developers.openai.com/codex
- 最新版本: https://github.com/openai/codex/releases/latest
- 94k Stars | 13.9k Forks | Apache-2.0 License

> 📁 本地 `docs/` 为上游仓库文档快照（vendored），整理时以线上文档为准；上游更新后如需同步，直接替换对应文件。

## Codex 产品矩阵

| 产品 | 说明 |
|------|------|
| **Codex CLI** | 终端本地编码代理 |
| **Codex IDE** | VS Code / Cursor / Windsurf 扩展 |
| **Codex App** | 桌面应用 (`codex app`) |
| **Codex Web** | 云端代理 (chatgpt.com/codex) |

## 目录

### 入门

- [简介](docs/start.md)
- [安装与启动](docs/install.md)
- [认证配置](docs/auth.md)
- [快速上手](docs/quickstart.md)

### 核心概念

- [配置系统](docs/config.md)
- [AGENTS.md 规则](docs/agents.md)
- [Skills 技能](docs/skills.md)
- [Plugins 插件](docs/plugins.md)
- [MCP 集成](docs/mcp.md)
- [Hooks 钩子](docs/hooks.md)

### 开发

- [从源码构建](docs/build.md)
- [项目结构](docs/architecture.md)
- [贡献指南](docs/contributing.md)
- [开源基金](docs/open-source-fund.md)

### 参考

- [命令行参考](docs/cli-reference.md)
- [故障排除](docs/troubleshooting.md)
